...
SFML::SFMLGroundPlot::SFMLGroundPlot(TurboHiker::Position2D position, std::pair<double, double>& size,
                                     const std::string& path_to_image, std::pair<int, int> dimensions,
                                     std::pair<int, int> imageCoordinates)
    : GroundPlot(position, size), SFMLEntity(path_to_image)
{
